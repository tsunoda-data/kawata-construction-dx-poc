...
施工現場における口頭発注、資材高騰見込み等の「確定前の予兆データ」は、IMPACT CONSTRUCTIONの確定データと統合されておらず、**着地予測の精度にギャップ**が生じている。また、過去データを活用したAIによる原価超過予測は未実装であり、予防的原価管理には至っていない。
...
加和太建設の最大の差別化要因は、建設→不動産→運営→エリア価値→新規受注の**循環的バリューチェーン**にある。本PoCでは、この循環構造を定量的に可視化するデータモデルを構築する。
...
| :--- | :--- | :--- |
| **事業別損益把握** | 各事業部が個別にP/L管理。グループ全体の事業ポートフォリオ分析は四半期ごとの手動集計。 | BigQueryに全事業データを統合し、**リアルタイムで事業別・エリア別の損益をクロス分析**可能に。 |
| **まちづくりROI** | 個別事業の投資回収は把握可能だが、「エリアへの総投資 vs 総リターン（建設受注＋不動産収益＋施設売上＋エリア価値向上）」の統合ROIは未計測。 | **エリア単位のROIダッシュボード**により、三島・沼津・函南など地域ごとの投資対効果を横断的に可視化。 |
| **相乗効果の定量化** | 「道の駅が集客に効いている」等の定性的な認識のみ。 | 施設来場者数 → 周辺不動産稼働率 → 新規受注件数の**相関分析・回帰分析**を自動化し、因果関係を数値化。 |

### 3.2 領域B：DXエコシステムのKPI管理

| 項目 | AS-IS（現状） | TO-BE（本システム導入後） |
| :--- | :--- | :--- |
| **SaaS事業KPI** | IMPACT CONSTRUCTIONのMRR・顧客数・チャーンレートは社内報告ベースで個別管理。 | **SaaS Growth Dashboard**にてMRR推移、LTV/CAC比率、Net Revenue Retentionを自動算出・可視化。 |
| **コミュニティ活動指標** | ON-SITE Xの参加者数・イベント実績は手動集計。ナレッジ共有（SCALE）の利用実態も定量化されていない。 | コミュニティ指標（参加社数、イベント開催数、マッチング成約数）をKPIとして統合モニタリング。 |
| **DX投資対効果** | DX推進室の活動成果は定性報告が中心。 | DX施策ごとの投入コスト vs 効果（工数削減時間、SaaS売上、受賞実績等）を**DX-ROI**として計測。 |

### 3.3 領域C：原価管理の高度化

| 項目 | AS-IS（現状） | TO-BE（本システム導入後） |
| :--- | :--- | :--- |
| **原価捕捉** | IMPACT CONSTRUCTIONで確定原価は管理。ただし口頭発注・資材高騰見込み等の**「予兆データ」は基幹システム外**で管理。 | 予兆データ入力UIを構築し、確定原価と予兆原価を統合した**実効原価率**をリアルタイム算出。 |
| **着地予測** | 実績ベースの線形外挿。季節性・工種特性・外部要因は考慮されていない。 | Vertex AI AutoMLによる**機械学習ベースの原価着地予測**。天候・資材価格指数・外注先稼働状況を特徴量として取り込み。 |
...
> **設計方針：** 既存の業務システム（特にIMPACT CONSTRUCTION）を**補完・拡張する位置づけ**であり、既存の業務フローを破壊しない。「車輪の再発明」を避け、既存資産の上にデータ統合・AI分析レイヤーを積む。
...
- 各エリアの**バリューチェーン連鎖図**（建設工事件数 → 施設来場者数 → 不動産稼働率 → 地価変動の相関）
...
> **PoC実装決定:** 単一の BigQuery データセット `kawata_dx_cockpit` にすべてのテーブルを集約する構成を採用。  
> Phase 2以降でデータ量・権限管理が複雑化した際に複数データセットへの分割を検討する。

```
{PROJECT_ID}.kawata_dx_cockpit
├── [ベーステーブル]
│   ├── projects_master      # 工事マスタ（pm_name カラム含む）
│   ├── cost_records         # 原価トランザクション（amount_kpy）
│   ├── cash_in_schedules    # 入金スケジュール
│   ├── real_estate_properties # 不動産物件マスタ
│   ├── facility_operations  # 施設月次運営実績
│   ├── saas_metrics         # SaaS事業KPI月次
│   └── area_indicators      # エリア指標月次
│
└── [分析ビュー（Looker Studio 参照先）]
    ├── vw_company_dashboard   # Page 1: 全社経営サマリー
    ├── vw_area_roi            # Page 2: まちづくりROI（エリア横断集計）
    ├── vw_area_indicators     # Page 2: エリア指標月次推移（DATE型変換）
    ├── vw_project_cost_summary  # Page 3: 工事別原価サマリー
    ├── vw_cost_category_detail  # Page 3: カテゴリ別予実明細（ドーナツ/棒グラフ用）
    ├── vw_cashflow_forecast   # Page 4: キャッシュフロー予測
    └── vw_ml_features         # AI/ML: 原価予測モデル学習用特徴量
```

**ビュー定義の全文:** [`sql/01_views.sql`](./sql/01_views.sql)

### 7.2 主要テーブル定義

> **カラム命名規則:**  
> - `_kpy`: 千円単位の金額 (例: `contract_amount_kpy`)  
> - `_rate`: パーセンテージ (例: `target_cost_rate`)  
> 完全な英日対照表 → [`docs/column_dictionary.md`](./docs/column_dictionary.md)

#### 【projects_master】工事マスタ

| カラム名 | 型 | 説明 |
| :--- | :--- | :--- |
| `project_id` | STRING (PK) | 工事ID（例: PRJ-0001） |
| `project_name` | STRING | 工事名称 |
| `division` | STRING | 事業区分（土木 / 建築） |
| `client_type` | STRING | 発注者区分（官公庁 / 民間） |
| `contract_amount_kpy` | INT64 | 請負契約金額（**千円**） |
| `target_cost_rate` | FLOAT64 | 目標原価率（%、例: 87.5） |
| `start_date` | DATE | 工期開始日 |
| `end_date` | DATE | 工期終了日 |
| `progress_rate` | FLOAT64 | 進捗率（%, 0.0-100.0） |
| `status` | STRING | ステータス（未着工 / 進行中 / 完了） |
| `pm_name` | STRING | **★ プロジェクトマネージャー名**（Looker Page 3で使用） |

#### 【cost_records】原価トランザクション

| カラム名 | 型 | 説明 |
| :--- | :--- | :--- |
| `record_id` | STRING (PK) | レコードID（例: CST-00001） |
| `project_id` | STRING (FK) | 工事ID |
| `category` | STRING | コスト区分（外注費 / 材料費 / 労務費 / 経費） |
| `amount_kpy` | INT64 | 金額（**千円**） |
| `status` | STRING | ステータス（確定請求 / 現場見込み / 口頭発注） |
| `accrual_month` | STRING | 計上対象月（YYYY-MM） |
| `payment_due_month` | STRING | 支払予定月（YYYY-MM）— CF異常検知で使用 |
...
| `expected_amount_kpy` | INT64 | 入金予定額（**千円**） |
| `actual_amount_kpy` | INT64 | 実入金額（**千円**、NULL許容） |
| `deposit_month` | STRING | 入金予定月（YYYY-MM）— vw_company_dashboard で使用 |
...
| `location` | STRING | 所在地（三島市 / 沼津市 / 函南町 等）— エリア判定に使用 |
| `acquisition_date` | DATE | 取得日 |
| `acquisition_cost_kpy` | INT64 | 取得価額（**千円**） |
| `current_value_kpy` | INT64 | 時価評価額（**千円**） |
| `monthly_rental_income_kpy` | INT64 | 月額賃貸収入（**千円**） |
| `occupancy_rate` | FLOAT64 | 稼働率（%） |
...
| `visitors` | INT64 | 来場者数 |
| `revenue_kpy` | INT64 | 売上高（**千円**） |
| `operating_cost_kpy` | INT64 | 運営費用（**千円**） |
| `operating_profit_kpy` | INT64 | 営業利益（**千円**） |
...
| `month` | STRING | 対象月（YYYY-MM）— ビューで DATE型に変換 |
| `land_price_index` | FLOAT64 | 公示地価指数（基準年=100） |
| `population` | FLOAT64 | 推計人口 |
| `foot_traffic` | FLOAT64 | 歩行者交通量 |
| `new_business_count` | INT64 | 新規開業事業所数 |
| `event_count` | INT64 | 地域イベント開催数 |

### 7.3 分析ビュー定義（Looker Studio 参照先）

> ビューの完全なSQL定義は [`sql/01_views.sql`](./sql/01_views.sql) を参照。  
> 各ビューのカラム英日対照は [`docs/column_dictionary.md`](./docs/column_dictionary.md) を参照。

| ビュー名 | 参照元テーブル | 主要カラム（★=新規追加） | Looker ページ |
| :--- | :--- | :--- | :--- |
| `vw_company_dashboard` | cash_in_schedules, cost_records, real_estate_properties, facility_operations, saas_metrics, projects_master | `sales_month`, `total_sales`, `actual_sales`, `forecast_sales`, `operating_profit_margin`, `active_project_count`, `cash_balance` | Page 1 |
| `vw_area_roi` | projects_master, real_estate_properties, facility_operations, area_indicators | `area`, `total_investment_kpy` ★, `roi_rate` ★, `total_return_kpy`, `land_price_index`, `population`, `foot_traffic` | Page 2 |
| `vw_area_indicators` ★ | area_indicators | `area_name`, `indicator_month`(DATE) ★, `land_price_index`, `population`, `foot_traffic`, `new_business_count` | Page 2 |
| `vw_project_cost_summary` | projects_master, cost_records | `pm_name` ★, `gross_profit` ★, `confirmed_cost_kpy`, `forecast_cost_kpy`, `effective_cost_rate`, `cost_rate_gap`, `risk_level` | Page 3 |
| `vw_cost_category_detail` ★ | cost_records, projects_master | `cost_category` ★, `category_budget` ★, `category_actual` ★, `category_variance` ★ | Page 3 |
| `vw_cashflow_forecast` | cash_in_schedules, cost_records | `cf_date`(DATE) ★, `beginning_balance_kpy` ★, `ending_balance_kpy` ★, `min_cash_flag` ★, `cashflow_risk` | Page 4 |
| `vw_ml_features` | projects_master, cost_records | `actual_cost_rate`, `confirmed_cost_rate`, `cost_progress_ratio`（XGBoost入力特徴量） | ML学習用 |
...
- [column_dictionary.md](./docs/column_dictionary.md) — **カラム英日対照表（v3で新規追加）**
- [sql/01_views.sql](./sql/01_views.sql) — BigQuery分析ビュー定義（7ビュー）
- [data/mock_data_generator.py](./data/mock_data_generator.py) — ダミーデータ生成スクリプト
- [analysis/](./analysis/) — 探索的データ分析・AIモデル構築ノートブック
- [dashboard/looker_studio_guide.md](./dashboard/looker_studio_guide.md) — **Looker Studio実装ガイド（v3で新規追加）**
